...
describe('middleware', () => {
  ...
  it('보호된 라우트에 토큰이 없으면 /login으로 리다이렉트한다', () => {
    const res = middleware(makeRequest('/my'));
  ...
  it('만료된 토큰이면 /login으로 리다이렉트한다', () => {
    ...
    const res = middleware(makeRequest('/my', token));
  ...
  it('손상된(디코딩 불가) 토큰이면 /login으로 리다이렉트한다', () => {
    const res = middleware(makeRequest('/my', 'not-a-valid-jwt'));
  ...
  it('USER role은 /merchant 접근 시 홈으로 리다이렉트한다', () => {
    ...
    const res = middleware(makeRequest('/merchant/1', token));
  ...
  it('ADMIN role은 /merchant, /admin 모두 통과한다', () => {
    ...
    expect(middleware(makeRequest('/merchant/1', token)).headers.get('location')).toBeNull();
    expect(middleware(makeRequest('/admin/users', token)).headers.get('location')).toBeNull();
  });
